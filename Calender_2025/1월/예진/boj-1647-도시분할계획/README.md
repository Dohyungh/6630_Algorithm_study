...
- 가장 극단적인 예시를 생각해봐야 한다. 집이 2개일 경우를 생각 안해서 첫 시도에 틀렸다.

### 설계 : 5분

- 주어진 간선들을 사용해 최소 신장 트리를 만든다. → 크루스칼
- 트리에서는 한 정점에서 다른 정점으로 가는 경로가 단 한개씩이다.
- 따라서 단 한개의 간선만 없애면 정점이 최소 하나인 두개의 최소신장트리로 나누어지므로, 최소 신장 트리를 이루는 간선 중 가장 긴 간선을 제외하면 된다.
- 최소신장트리를 이루는 간선의 개수는 (정점개수 - 1) 개 이기 때문에 크루스칼 방식에서 사용된 간선의 개수가 (정점개수 - 2) 개인 경우까지만 더하면 답을 구할 수 있다.