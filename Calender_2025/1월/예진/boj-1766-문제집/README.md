...
- 위상정렬인데 내부에서 조건에 따라 우선순위가 계속 바뀌는 형태라고 생각했다.
- 딱 처음 우선순위큐를 떠올렸는데, 순간 착각해서 우선순위큐로 하면 먼저 큐에 들어간 문제보다 다른문제가 더 빨리 나올 수도 있겠다 싶어서(←사실 이게 맞다) 다음에 풀 문제들을 리스트에 담아뒀다가 정렬해서 방문하는 방법으로 했다. 예제 틀림.
- 엥? 싶어서 보니 우선순위큐를 쓰는게 맞았다,,ㅋㅋ 이렇게 쉬울리가 없다고 일부러 돌려 풀었는데 독이었다

### 설계 : 10분

- 입력되는 문제 간의 관계를 int[] enter 배열과 다음순서 문제 List[] next로 생성한다.
- 기초 위상정렬과 달리 큐가 아니라 우선순위 큐를 선언해서 진입차수가 0인 문제를 순회한다.
- 문제를 풀며 다음 순서인 문제들의 진입차수를 -1하여 진입차수가 0이 되는 다음 문제들을 우선순위큐에 넣는다.
- 방문하는 순서대로 출력한다.