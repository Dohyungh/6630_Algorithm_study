...
- 순서가 있어서 바로 위상정렬로 설계를 했다.
- 단, 문제 입력조건을 제대로 확인하지 않아서 틀릴뻔했다..

### 설계 : 5분

- 입력되는 PD 별 정한 가수들의 순서를 순회하며 i다음 i+1이 오도록 진입차수 배열(enter)와 인접리스트(adj)를 생성한다.
- 순서를 결정했는지(결정할 수 있었는지) 확인하는 boolean[] confirm 배열도 준비한다.
- 진입차수가 0인 대상을 큐에 넣어 다음 순서인 가수들의 진입차수를 -1한다.
- 윗줄을 반복하며 순서가 정해진 가수들을 StringBuilder에 추가한다.
- 위상정렬 순회가 끝났을때 한명이라도 confirm = false이면 순서를 정할 수 없는 경우가 된다.