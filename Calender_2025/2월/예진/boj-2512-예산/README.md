...
- 나무자르기랑 비슷했는데, 두 문제를 풀면서 이분 탐색시 조건에 따른 분기 경험을 할 수 있어서 좋았다.

### 설계 : 5분

- 지방의 예산을 배열에 저장한다.
- 1부터 가장 높은 예산을 범위로 하여 이분탐색을 진행한다.
- mid 값이 국가 총예산과 같으면 해당 값을 answer에 저장한다.
- mid 값이 총예산보다 적으면 일단 값을 answer에 저장하고 low = mid+1로 변경한다.
- mid 값이 총예산보다 크면 high = mid-1로 변경한다.
- 가능한 최대값을 answer에 일단 저장하기 때문에 left>right 으로 반복문이 끝나도 이미 답이 저장되어 있다.