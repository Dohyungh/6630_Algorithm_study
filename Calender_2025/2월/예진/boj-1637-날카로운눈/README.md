...
- answer 초기화 위치를 잘못 잡아서 정수 더미의 수가 연속된 2개의 숫자뿐일 때의 경우를 고려하지 못했다. 디버깅 지옥ㅠ

### 설계 : 30분

- 매개변수 이분탐색: mid를 기준으로 두 파트로 나뉜 정수의 전체 개수를 세어 홀수인 쪽으로 범위를 좁혀간다.
- 처음 시작시 전체 개수가 짝수이면 홀수개인 정수가 없으므로 NOTHING을 출력한다.
- 두 파트의 정수의 개수는 유효한 구간을 순회하며 `(high - low) / B + 1;` 를 계속 더하면 각 조건에 맞는 정수 개수를 구할 수 있다.
    - 단, high - low가 음수일때는 통과한다.