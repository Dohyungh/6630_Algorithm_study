...
- 예전에 스터디에서 dp문제라는 걸 들은 것이 기억나서 비교적 쉽게 접근했다. 그 부분이 조금 아쉽다.
- 접근법을 몰랐다면 잘 풀었을지는 미지수다.

### 설계 : 40분

- 정삼각형, 마름모 도형을 사용해 **사다리꼴을 만들 수 있는 경우**는 2가지
    1. 사다리꼴 + 마름모(정삼각형 2개)
...
- 정삼각형, 마름모 도형을 사용해 마름모를 만들 수 있는 경우는 2가지
    1. 사다리꼴 + 정삼각형 1개
...
- 이 규칙을 생각하며 윗변에 정삼각형이 올라가는 경우에 차이점을 반영해 점화식을 분기한다.
    - int[][] dp = new int[2][n+1];
    - dp[0][j] : 윗변의 길이가 j인 사다리꼴 경우의 수
    - dp[1][j] : 윗변의 길이가 j인 마름모 경우의 수
