...
- 시간복잡도에서 걸릴 줄 알았는데, 계산해보니 30개 이하의 숫자로 5개 숫자 조합을 만드는 방법은 7만개 정도였다..아님 말고,,,
    - (30*29*28*27*26) / (5*4*3*2*1)

### 설계 : 10분

- dfs로 숫자의 조합을 구하고, 해당 숫자 조합이 질문의 조합들과 비교했을 때 각각 답과 동일한 개수만큼 존재하는지 체크했다. (즉, 모든 질문과 답에 유효한 조합인지)