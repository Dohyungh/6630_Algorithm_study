...
- 문제를 잘못 이해해서 너무 헤맸다.
- 정점의 개수와 진입, 진출 차수, 인접리스트 초기화를 좀 더 효율적으로 할 방법이 없을까 싶다.

### 설계 : 120분

- 간선 정보를 통해 각 정점에 대한 진입/진출 차수를 각각 구하고, 인접 리스트를 기록한다.
- 진입 차수가 0이고, 진출 차수가 2 이상인 정점이 새로 생성된 정점이다.
- 새로 생성된 정점이 향하는 모든 그래프를 판단한다.
    - 탐색 중에 진출 차수가 2이상인 정점을 만나면 8자 그래프
    - 탐색 중에 방문했던 정점에 다시 방문한 경우 도넛 그래프
    - 탐색 중에 더이상 다음 정점이 없는 경우 막대그래프