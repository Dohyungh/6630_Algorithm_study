...
- 트리라는 말에 부모→자식 단방향으로만 인접리스트를 생성했는데, 트리의 지름을 구하려면 양방향으로 탐색할 수 있어야 한다. 주의!

### 설계 : 10분

- 모든 노드에 대해 인접리스트를 생성한다.
- 임의의 한 지점에서 출발해서 가장 먼 노드의 번호를 저장한다. (다익스트라)
- 저장된 번호에서 시작해서 가장 먼 노드까지의 거리를 구한다. (다익스트라)