...
- 그냥 방문체크를 따로하는 dfs같은데 왜 골드4일까

### 설계 : 5분

- 알파벳 26개의 방문체크를 하는 visited를 생성한다.
- board에는 각 알파벳에서 ‘A’ 아스키코드값을 빼서 0~25의 인덱스를 입력하도록 한다.
- 시작지점부터 dfs를 통해 가장 많이 갈 수 있는 지점까지의 이동 거리를 갱신한다.