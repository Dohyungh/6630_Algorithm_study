...
- 답을 참고하지 않고 스스로 응용해서 풀어내서 기뻤다.

### 설계 : 30분

- 배낭문제처럼 풀면 될 것 같았는데, 상담별로 기간을 채워야 하는게 다른 점이었다.
- 하나의 상담일정을 검토하면서 각 날짜마다의 최대 수익을 갱신했다.
- 갱신이 유효한 시점을 `상담 시작일 + 상담기간`으로 했다. (이전까지는 앞까지의 값을 그대로 가져옴)
- 갱신되는 값은 `(상담 시작일 - 1)의 값 + P[상담 시작일]` 로 했다.
    - 이전의 값과 비교해서 최대값으로 갱신한다. (지금 검토하는 상담을 하는 결과 or 하지 않는 결과 중 큰 값)
- 갱신 이후의 값은 이전 날의 값과 이전 상담을 검토한 당일의 값 중 최대값으로 갱신한다.
    - (지금 검토하는 상담을 포함해서 갱신한 결과 or 포함하지 않은 결과) 중 최대값≈