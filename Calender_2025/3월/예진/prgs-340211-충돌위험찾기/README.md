...
- 문제를 이해하고 코드화 하기에 좀 복잡했다.
- bfs인가 했는데, 사실 그래프 탐색까진 필요하지 않았다.
- 같은 칸에 여러 로봇이 모이면 각각의 충돌위험을 다 세어야 하는줄 알았는데 아니었다..괜히 복잡하게 생각했다.)

### 설계 : 15분

- 목적지와의 좌표 차이에서 먼저 이동해야 하는 방향이 정해져 있기 때문에 bfs는 필요없다.
    - 경로가 이미 정해져 있다. 비교 연산을 통해 구해야 할 뿐.
- 각 시간마다 모든 로봇을 순회하며 이동해야 하는 방향으로 한칸씩 이동한다.
    - r좌표가 다르다면 r좌표 먼저 - 또는 +, c좌표만 다르다면 c좌표를 - 또는 +
- 이동하려는 위치에 로봇 개수를 +1 하고, 그 수가 2가 될때만 충돌 감지 횟수를 올린다. (처음 감지될때만)
- 이동한 위치가 목표한 위치와 같아지면 목적지 순서 인덱스를 +1하여 목적지를 변경한다.
    - 가야할 목적지의 인덱스가 루트의 길이보다 길어지면 해당 로봇은 end 처리한다.