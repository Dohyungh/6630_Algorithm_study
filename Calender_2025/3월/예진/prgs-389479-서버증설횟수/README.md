...
- 24길이의 서버 현황 배열을 생성한다. (int[] servers)
1. players 배열과 비교하여 players[i] / m 개수보다 servers[i]개수가 더 적으면 서버 증설이 필요하다.
2. 필요한 개수((players[i] / m) - servers[i])만큼 i부터 i+k이전까지 서버수를 추가한다.
3. 배열을 쭉 순회하며 1, 2번을 반복한다.