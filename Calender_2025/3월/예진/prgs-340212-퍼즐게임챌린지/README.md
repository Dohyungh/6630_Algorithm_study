...
- 문제에서 접근법에 대한 힌트가 잘 느껴져서 쉽게 풀었다.
- 문제의 규칙을 설명하는 구성이 그냥 매개변수 이분탐색 풀이처럼 보였다.

### 설계 : 5분

- 설정 가능한 레벨의 범위를 정한 후(1~100,000), 이분탐색을 실행하여 limit 안에 해결 가능한 최소 레벨을 찾는다.
- mid값을 사용해 게임의 규칙대로 모든 문제를 순회하며 소요되는 시간을 구한다.
    - 이때, 소요시간이 limit보다 커지면 false를 반환한다.
- 나는 mid값이 가능하다면 high = mid-1 하기 때문에 해당 mid값을 다시 확인할 길이 없어 answer에 바로 저장했다.
- 또한 low = high-1 인 상황에서 mid = low가 될 것이고, 불가능으로 판단될 경우 low = high 가 될 것이기 때문에 탐색 조건을 (low ≤ high)로 설정했다. (high도 가능 여부를 판단해야 하기 때문)