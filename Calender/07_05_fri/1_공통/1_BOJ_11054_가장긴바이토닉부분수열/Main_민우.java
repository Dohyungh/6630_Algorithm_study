...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        // int max = IntStream.range(0, N).map(i -> dp1[i] + dp2[i] - 1).max().getAsInt();    // 한줄로도 되는데 훨씬 느림
        // int max = IntStream.range(0, N).parallel().map(i -> dp1[i] + dp2[i] - 1).max().getAsInt();    // 병렬 처리
